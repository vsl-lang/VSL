...
ClassStatement[s] -> Modifier "class" _ classDeclaration _ (":" _ ExtensionList _ {% nth(2) %}):? "{" _ (ClassItems[$s] _ {% id %}):? "}" {%
...
InterfaceStatement[s] -> Modifier "interface" _ classDeclaration _ (":" _ ExtensionList _ {% nth(2) %}):? "{" _ (InterfaceItems[$s] _ {% id %}):? "}" {%
