...
export default class ParserServer extends CompilerCLI {
    ...
    run(args) {
        ...
        for (let i = 0; i < args.length; i++) {
            if (args[i][0] === "-" && args[i].length > 1) {
                ...
                if ('color' in flagInfo) color = args[++i];
    ...
    startWorker(port) {
        ...
        const server = net.createServer({
            ...
            this.printLog(`Instance ${instance} received`, 'status');
            ...
            // When all done
            c.on('close', () => {
                this.printLog(`Instance ${instance} completed`, 'status');
            });

            // When all data is received
            c.on('end', () => {
                this.printLog(`Instance ${instance} processed ${data.length} bytes`, 'status');
        ...
        server.listen(port, () => {
            ...
            this.printLog(`Worker ${cluster.worker.id} at ${location}`, 'status');
        });
     ...
     */
    startServer(port) {
        this.printLog(`Starting on port ${port}`, 'status');
